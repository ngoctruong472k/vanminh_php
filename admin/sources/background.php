...
function get_gioithieu(){
	...
	if($d->num_rows()==0)
	{
		...
		if($d->insert($data))
			transfer(_dulieukhoitao,"index.php?com=background&act=capnhat&type=".$_REQUEST['type']);
		else
			transfer(_dulieukhoitaoloi,"index.php?com=background&act=capnhat&type=".$_REQUEST['type']);
	}
	$item = $d->fetch_array();
}
function save_gioithieu(){
	global $d,$config;
	if(empty($_POST)) transfer(_khongnhanduocdulieu, "index.php?com=background&act=capnhat&type=".$_REQUEST['type']);
	...
	$d->setWhere('type', $_REQUEST['type']);
	if($d->update($data))
		transfer(_dulieuduoccapnhat, "index.php?com=background&act=capnhat&type=".$_REQUEST['type']);
	else
		transfer(_capnhatdulieubiloi, "index.php?com=background&act=capnhat&type=".$_REQUEST['type']);
}
