...
function get_item(){
	...
	if(!$id)
		transfer(_khongnhanduocdulieu, "index.php?com=com&act=man".$urlcu);
	
	$sql = "select * from #_com where id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer(_dulieukhongcothuc,"index.php?com=com&act=man".$urlcu);
	$item = $d->fetch_array();
}
//===========================================================
function save_item(){
	...
	if(empty($_POST)) transfer(_khongnhanduocdulieu, "index.php?com=com&act=man".$urlcu);
	...
	if($id){
		...
		if($d->update($data))			
			...
			transfer(_capnhatdulieubiloi, "index.php?com=com&act=man".$urlcu);
	}else{
		...
		if($d->insert($data))
			...
			transfer(_luudulieubiloi, "index.php?com=com&act=man".$urlcu);
	}
}
//===========================================================
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		if($d->query($sql))
			...
			transfer(_xoadulieubiloi, "index.php?com=com&act=man".$urlcu);
		...
		} redirect("index.php?com=com&act=man".$urlcu);} else transfer(_khongnhanduocdulieu, "index.php?com=com&act=man".$urlcu);
}
