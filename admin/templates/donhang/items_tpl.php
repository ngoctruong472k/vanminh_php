...
$("#exporthet").click(function(){
	...
	listid=listid.substr(1);	 //alert(listid);
	if (listid=="") { alert("You have not selected any orders"); return false;}
	hoi= confirm("Are you sure you want to export?");
...
<div class="widget">
  <div class="titlee" style="padding-bottom:5px;">
    ...
    <form name="search" action="index.php" method="GET" class="form giohang_ser">
      ...
      <input class="form_or" name="keyword" placeholder="<?=_nhaptukhoatimkiem?>" value="<?=$_REQUEST['keyword']?>" type="text" />
      <input class="form_or" name="ngaybd" id="datefm" type="text" value="<?=$_REQUEST['ngaybd']?>" placeholder="From the date.."/>
      <input class="form_or" name="ngaykt" id="dateto" type="text" value="<?=$_REQUEST['ngaykt']?>" placeholder="To date.." />
      ...
      <select name="httt">
      <option value="0"><?=_hinhthucthanhtoan?></option>
        <?php 
          $sql="select id,ten$lang as ten from #_httt order by id";
          $d->query($sql);
      ...
      <option value="0"><?=_tinhtrang?></option>
        <?php 
          $sql="select id,trangthai$lang as trangthai from #_tinhtrang order by id";
          $d->query($sql);
...
<div class="widget" style="overflow-y: auto;">
  <div class="title" style="min-width: 842px;"><span class="titleIcon">
    <input type="checkbox" id="titleCheck" name="titleCheck" />
    </span>
    <h6><?=_danhsachdonhang?></h6>
  </div>
  <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll" 
  style="min-width: 842px;">
    <thead>
      <tr>
        <td></td>
       <td class="sortCol" width="120"><div><?=_madonhang?><span></span></div></td>     
        <td class="sortCol"><div><?=_hovaten?><span></span></div></td>
        <td class="sortCol" width="150"><div><?=_ngaydat?><span></span></div></td>
        <td width="150"><?=_sotien?></td>
        <td width="150"><?=_tinhtrang?></td>
    ...
    <tbody>
         <?php for($i=0, $count=count($items); $i<$count; $i++){?>
          <tr <?php if($items[$i]['tinhtrang']==1){ ?>style="font-weight: bold;"<?php } ?>>
       <td>
        ...
        <td align="center"><?=number_format($items[$i]['tonggia']+(($items[$i]['nhanhang']==0&&$items[$i]['quan']!=0)?Get_PhiShipTheoQuan($items[$i]['quan']):0),0, ',', '.')?>&nbsp;đ
         <?php /*
					$d->reset();
					...
					$tongtiendonhang = 0 + $items[$i]['phithem'];
					$soluongchiphi = 0;
				?>
                 ...
                 <?=number_format($tongtiendonhang,0, ',', '.')."&nbsp;đ" */?>
        </td>
