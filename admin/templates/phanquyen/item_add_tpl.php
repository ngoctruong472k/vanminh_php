	...
	<div class="widget">
       ...
       <div id="info" class="tab_content">
	        ...
	        <div class="formRow">
	            <label><?=_thutu?>: </label>
	            <div class="formRight">
	                <input type="text" class="tipS" value="<?=isset($item['stt'])?$item['stt']:SoThuTu_ASC('phanquyen','')?>" name="stt" style="width:20px; text-align:center;" onkeypress="return OnlyNumber(event)" original-title="Số thứ tự của danh mục, chỉ nhập số">
	            </div>
	            <div class="clear"></div>
	        </div>
	...
	$(document).ready(function(e) {
		...
		$('button.add-permission').click(function(){
			...
			$.ajax({
				...
				type:'post',
				dataType:'json',
				data:{id:'<?=$_REQUEST['id']?>',com:com,type:type,man:man,add:add,edit:edit,delete2:delete2,act_cap:act_cap},
				success:function(kq){
					if(kq.thongbao==1)
					{
...
<div id="permission-draw"><div class="widget">
	<table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable phanquyen" id="checkAll">
        ...
        <?php
			...
			$d->reset();
			$sql = "select act from #_com_quyen where id_quyen='".$_REQUEST['id']."' and com='".$item_com[$i]['com']."' and act_cap='".$act_cap."' and type='".$item_com[$i]['type']."' limit 0,1";
			$d->query($sql);
        ...
        <tr class="tr_pquyen">
            ...
            <td style="text-align:center;"><button class="button blueB add-permission"><i class="glyphicon glyphicon-upload"></i><?=_xacnhan?></button></td>
         </tr>  
        <?php } ?>  
	</table>
</div>
