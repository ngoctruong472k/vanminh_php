...
function select_onchange2()
  {
    ...
    window.location ="index.php?com=place&act=man_ward&id_city="+a.value+"&id_dist="+b.value+"&id="+c.value; 
    return true;
  } 			
</script>
<?php 
  ...
  {
    $sql_huyen="select * from table_place_ward where id_dist='".$_REQUEST['id_dist']."' order by stt,id asc";
    $result=mysql_query($sql_huyen);
    $str='
      <select id="id" name="id" onchange="select_onchange2()" class="main_select select_danhmuc">
      <option value="">Chọn tên phường/xã</option> 
      ';
    while ($row_huyen=@mysql_fetch_array($result)) 
    {
  ...
  <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
    <thead>
      <tr>
        ...
        <td class="tb_data_small"><a href="#" class="tipS" style="margin: 5px;">Thứ tự</a></td>      
         ...
         <td class="sortCol"><div><?=get_main_ward(@$item['id'])?><span></span></div></td>
         <td class="sortCol"><div>ID phường,xã<span></span></div></td>
    ...
    <tbody>
       ...
       <td>
        ...
        <td align="center">
            <input data-val0="<?=$items[$i]['id']?>" data-val2="table_<?=$_REQUEST['com']?>_ward" data-val3="stt" onblur="stt(this)" type="text" value="<?=$items[$i]['stt']?>" name="ordering[]" onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')" class="tipS smallText update_stt" original-title="Nhập số thứ tự bài viết" rel="<?=$items[$i]['id']?>" />
        </td> 
      ...
      ?>      
        ...
        <td class="title_name_data">
            <a href="index.php?com=place&act=edit_ward&id=<?=$items[$i]['id']?>&id_city=<?=$items[$i]['id_city']?>&id_dist=<?=$items[$i]['id_dist']?>" class="tipS SC_bold"><?=$items[$i]['id']?></a>
        </td>
       
        <td align="center">
           <a data-val2="table_<?=$_REQUEST['com']?>_ward" rel="<?=$items[$i]['hienthi']?>" data-val3="hienthi" class="diamondToggle <?=($items[$i]['hienthi']==1)?"diamondToggleOff":""?>" data-val0="<?=$items[$i]['id']?>" ></a>
        </td>
