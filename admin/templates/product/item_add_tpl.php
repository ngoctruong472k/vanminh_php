...
function get_main_thuonghieu()
    {
        $sql="select * from table_product where type='thuong-hieu' order by stt";
        
        $stmt=mysql_query($sql);
        $str='
            <select id="thuonghieu" name="thuonghieu" class="main_select select_danhmuc select_dmbaiviet" data-level="6" data-type="thuong-hieu" data-child="id">
            <option value="0">Chọn thương hiệu</option>         
            ';
        while ($row=@mysql_fetch_array($stmt)) 
        {
            if($row["id"]==(int)@$_REQUEST["thuonghieu"])
                $selected="selected";
            else 
                $selected="";
            $str.='<option value='.$row["id"].' '.$selected.'>'.$row["ten"].'</option>';            
...
<div class="control_frm" style="margin-top:25px;">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
        	            <li><a href="index.php?com=product&act=man<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>"><span><?=_sanpham?></span></a></li>
...
<form name="supplier" id="validate" class="form" action="index.php?com=product&act=save&p=<?=$_REQUEST['p']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" method="post" enctype="multipart/form-data">
 	...
 	<div id="info" class="tab_content">
        ...
        <div class="formRow">
         ...
         <div class="formRow">
            <label><?=_giacu?>: </label>
            <div class="formRight">
                <input type="text" id="giacu" name="giacu" value="<?=@$item['giacu']?>"  title="Nhập giá cũ sản phẩm" class="tipS conso" onkeypress="return OnlyNumber(event)" />
            </div>
            <div class="clear"></div>
        </div>    
        ...
        <div class="formRow">
            <label><?=_giaban?>: </label>
            <div class="formRight">
            	<input type="text" id="giaban" name="gia" value="<?=@$item['gia']?>"  title="Nhập giá bán sản phẩm" class="tipS conso" onkeypress="return OnlyNumber(event)" />
            </div>
            <div class="clear"></div>
        </div>
		
		<div class="formRow">
            <label><?=_giaban?> sỉ: </label>
            <div class="formRight">
            	<input type="text" id="giasi" name="giasi" value="<?=@$item['giasi']?>"  title="Nhập giá bán sỉ sản phẩm" class="tipS conso" onkeypress="return OnlyNumber(event)" />
            </div>
            <div class="clear"></div>
        </div>
		
		<div class="formRow">
            <label>Số lượng sỉ: </label>
            <div class="formRight">
            	<input type="text" id="soluongsi" name="soluongsi" value="<?=@$item['soluongsi']?>"  title="Nhập số lượng sỉ sản phẩm" class="tipS conso" onkeypress="return OnlyNumber(event)" />
            </div>
            <div class="clear"></div>
        </div>
        ...
        <?php
        	$d->reset();
			$sql="select id from table_product_size where type='".$_REQUEST['type']."'";
			$d->query($sql);
        ...
        <div class="formRow ">
            	<?php 
					$d->reset();
					$sql = "select ten$lang as ten,id,tenkhongdau from #_product_size where hienthi=1 and type='".$_REQUEST['type']."' order by stt,id desc";
					$d->query($sql);
					$list_size = $d->result_array();
				?>
                <label><?=_chonsize?></label>
                <?php $tach_size = explode(',', @$item['id_size']) ?>
                <div class="formRight">
                    <select name="id_list_chon[]" multiple class="chosen-select input form-control" placeholder="<?=_chonsize?>">
                            <?php foreach ($list_size as $key => $value) { ?>
                                <?php if (count($tach_size)>0 && $tach_size!="") { ?>
                                    <?php $selected=""; ?>
                                    <?php foreach ($tach_size as $key => $check) { ?>
        ...
        <div class="formRow">
			...
			<div class="formRight">
				...
				<div class="note"> Min-width:440px | Min-height:450px <?=_format_duoihinh_l?> </div>
			</div>
			<div class="clear"></div>
		</div>
         <?php if($_REQUEST['act']=='edit'){?>
    ...
    <?php }?>
      <div class="formRight">
          <a class="file_input" data-jfiler-name="files" data-jfiler-extensions="jpg, jpeg, png, gif"><i class="fa fa-paperclip"></i> <?=_themanh?></a>                
        ...
        <div class="formRow none">
         ...
         <div class="formRight">
            <input type="checkbox" name="spbanchay" id="check3" <?=( $item['spbanchay']==1)?'checked="checked"':''?> />
            </div>
            <div class="clear"></div>
          </div>
        <div class="formRow none">
         ...
         <div class="formRight">
            <input type="checkbox" name="tieubieu" id="check4" <?=( $item['tieubieu']==1)?'checked="checked"':''?> />
            </div>
            <div class="clear"></div>
          </div>
        <div class="formRow none">
         ...
         <div class="formRight">
            <input type="checkbox" name="spmoi" id="check5" <?=( $item['spmoi']==1)?'checked="checked"':''?> />
            </div>
            <div class="clear"></div>
          </div>
