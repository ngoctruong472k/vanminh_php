...
<script>
    $(document).ready(function(e){
        $('.danhmuc1').each(function(){
            $(this).find('.danhmuc11:first').addClass('active');
            var a = $(this).find('.danhmuc11:first');
            var id = a.data('id');
            $.ajax({
                url: 'ajax/sanpham1.php',
                type: 'POST',
                dataType: 'html',
                cache: false,
                data: {id:id},
        ...
        $(".danhmuc1 a").click(function(){
            $(this).parent('.danhmuc1').children('.danhmuc11').removeClass("active");
...
.ajax_paging div.active {background: #189eff; color: #f5faf5; cursor:pointer;
    pointer-events: none;}
